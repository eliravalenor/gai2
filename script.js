...
async function testApiConnection() {
    ...
    try {
        ...
        const models = data.data ? data.data.map(m => (m.id || m.name || m)).map(x=>String(x)).filter(Boolean).sort() : [];
if (models.length === 0) {
    // 强制兜底：给出 Gemini 常用模型
    const fallback = ['gemini-2.5-pro','gemini-2.5-flash','gemini-2.0-pro','gemini-2.0-flash','gemini-1.5-pro','gemini-1.5-flash'];
    fallback.forEach(id => models.push(id));
}
if (models.length === 0) {
